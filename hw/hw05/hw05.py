...
def replace_leaf(t, old, new):
    ...
    for item in branches(t):
        newbranches.append(replace_leaf(item,old,new))
...
def move_stack(n, start, end):
    ...
    "*** YOUR CODE HERE ***"
    mid=6-start-end #calculate the mid point
    if n==1:
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,input):
        nonlocal balance,attempts,locked,password
        if locked:
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount,input):
        if input==old_password or input==new_password:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y)>0 or upper_bound(y)<0, "must not span zero"
...
def check_par():
    ...
    r2 = interval(0.1, 0.2) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    def f(t,a,b,c):
        return a*t*t+b*t+c
    left=f(lower_bound(x),a,b,c)
    right=f(upper_bound(x),a,b,c)
    extreme_point=-b/(2*a)
    if lower_bound(x)<extreme_point<upper_bound(x):
        mid=f(extreme_point,a,b,c)
